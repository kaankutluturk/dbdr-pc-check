...
| Extended forensic metadata | Opt-in capped Amcache executable inventory, time-bounded Application Error/PowerShell lifecycle events, bounded NTFS executable-change history and minimized SRUM executable-usage buckets | SRUM retains only executable name/redacted path/timestamp after parsing; user IDs, network fields, usage amounts, USN parent paths/file references, event payloads and dumps are excluded |
