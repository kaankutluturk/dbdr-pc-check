...
- When **Extended forensic metadata** is explicitly selected: Windows `powercfg /srumutil` application-usage output, capped at 64 MiB, 60 seconds and 5,000 emitted records. Only an execution-capable application filename, redacted path where resolved, identity form and in-window source timestamp are serialized. User/SID fields, network/interface/destination fields, resource or byte amounts, opaque identifiers and unrelated application rows are discarded.
- Exact redacted executable-path correlations across approved BAM, Amcache or minimized SRUM usage and live-module/persistence binary records. A path correlation is emitted only when the binary also has an independent non-valid signature, YARA match or PE-import review signal; it remains a review lead, not a verdict.
...
New v0.5.0 UI collections are written as authenticated encrypted `.dbdr` files. The application uses chunked AES-256-GCM with a random salt/nonce prefix and a PBKDF2-SHA256 passphrase-derived key; the passphrase is not written into the bundle and cannot be recovered by DBDR. The reader verifies authenticated encryption, bounded archive structure, the complete SHA-256 manifest and the supported schema before loading evidence. Legacy plaintext ZIPs remain readable for migration and must be treated as unencrypted.

Packaging and reopening can briefly create plaintext working data under the current Windows temporary directory. The opt-in SRUM source also asks the Microsoft `powercfg` utility to create a full native SRUM XML export in a unique temporary directory before DBDR minimizes it; that temporary source can contain fields DBDR deliberately excludes from the bundle. The application enforces size/time limits and performs immediate best-effort cleanup, but an abnormal process or system failure can leave recoverable local remnants. The encrypted result can still contain sensitive system and application metadata. Do not post bundles or passphrases in public Discord channels, GitHub issues or other public locations, and do not transmit the passphrase in the same channel as the bundle.

Before production use, the organization operating the check must document its controller, purpose, lawful basis, case-scoped access list, retention period, deletion procedure and appeal path. Retain a bundle only as long as the case requires, delete working copies and exports when that period ends, and record any authorized disclosure. Any future upload must preserve authenticated encryption and use explicit case-scoped authorization. A production client must never embed a private signing or decryption key.
