...
- **SRUM/powercfg:** the opt-in adapter depends on the installed Windows `powercfg /srumutil` export schema and SRUM retention. Bucket timestamps are not precise process-start times, identities can be unresolved, and a missing row is not proof of non-use. DBDR serializes only executable identity and an in-window bucket timestamp, but the native utility first writes a full plaintext temporary XML export; a crash can leave excluded fields recoverable until the operator's normal temporary-file cleanup.
- **Path correlation:** exact string equality between already redacted paths does not resolve junctions, aliases, replacement files or timestamp manipulation. The profile requires a separate signature, YARA or PE-import signal, and the result remains a corroboration lead rather than attribution.
...
- **YARA:** rule quality, scope and version directly control false positives and false negatives. Raw operator rules are labeled unverified. Offline `.dbdrrules` packs fail closed on unexpected ZIP entries, traversal, caps, invalid UTF-8, includes, rules-hash mismatch, profile mismatch, expiry, unknown keys or invalid ECDSA P-256/SHA-256 signatures; only public trust keys are embedded. This authenticates the publisher and bytes, not rule quality. The portable client intentionally has no persistent anti-rollback database, so operations must retire older still-valid versions. The suite records ruleset hashes, trust labels and rule identifiers so a reviewer can reproduce and challenge a match. Because the managed wrapper does not expose YARA's native timeout API, production scans run in a killable same-EXE worker with a 20-second per-file deadline. Inputs and reported matches are capped. A match never creates a verdict.
